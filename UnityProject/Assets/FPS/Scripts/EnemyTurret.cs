...
public class EnemyTurret : MonoBehaviour
{
    ...
    [Tooltip("The the time it takes the turret to spawn another hover bot")]
    public int hoverBotSpawnInterval = 5;

    private float timerSinceHoverBotSpawn = 0;
    ...
    void UpdateHoverSpawner()
    {
        timerSinceHoverBotSpawn += Time.deltaTime;
        if (timerSinceHoverBotSpawn > hoverBotSpawnInterval)
        {
            var newBot = Instantiate(hoverBot);
            newBot.transform.position = transform.position + new Vector3(Random.Range(-10,10),Random.Range(-10,10),0);
            timerSinceHoverBotSpawn = 0;
        }
    }
