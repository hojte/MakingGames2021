...
public class ForkliftController : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Keypad1))
            AudioUtility.CreateSFX(onDamage, transform, 1f, 0.5f, 15f); // todo sound
    }
    ...
    public void die()
    {
        if (Time.time > (lastHitTime + 3.0f))
        {
            ...
            else if (hp <= 0)
            {
                ...
                AudioUtility.CreateSFX(onDeath, transform, 1f, 0.5f, 15f); //todo sound
    ...
    {
        AudioUtility.CreateSFX(onHit, transform, 1f, 0.5f, 15f); //todo sound
    }
}